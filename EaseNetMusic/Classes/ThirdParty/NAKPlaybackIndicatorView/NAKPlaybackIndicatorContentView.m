...
- (void)prepareBarLayers
{
    ...
    for (NSInteger i = 0; i < self.style.barCount; i++) {
        CALayer* layer = [self createBarLayerWithXOffset:xOffset Index:i];
...
- (CALayer*)createBarLayerWithXOffset:(CGFloat)xOffset Index:(NSInteger)index
{
    ...
    CGFloat idleBarHeight = self.style.idleBarHeight;
    if (index == 1) {
        idleBarHeight = self.style.maxPeakBarHeight;
    }else if (index == 2) {
        idleBarHeight = self.style.idleBarHeight + 3.0;
    }else if (index == 3) {
        idleBarHeight = self.style.maxPeakBarHeight - 3.0;
    }
    layer.bounds = CGRectMake(0.0, 0.0, self.style.barWidth, idleBarHeight);// In its own coordinate
    layer.allowsEdgeAntialiasing = YES;
    layer.cornerRadius = self.style.barWidth/2.0;
    return layer;
}
