...
//        tabBar.isTranslucent = false
    ...
    func addChildVC(_ childVC: UIViewController, title:String?, image:String, selectedImage:String) {
        ...
        childVC.title = title
        childVC.tabBarItem.setTitleTextAttributes([.foregroundColor: UIColor.red], for: .selected)
...
// MARK: - 让当前显示的控制器控制statubar的样式和隐藏
    ...
    //            if let vc = viewController as? CTBaseController {
                    //add refresh list code here...
    //            }
            }
            print("will select vc: \(viewController.title ?? "")")
            return true
        }
